...
func (d *biodiversity) GetByID(id int) (*entity.BiodiversityResponse, error) {
	...
	if err := d.db.Table("biodiversities").
		Select("biodiversities.*, species.name as species, genus.name as genus, familia.name as familia, ordos.name as ordo, classes.name as classis, kingdoms.name as kingdom, phylums.name as phylum, diovisios.name as diovisio, statuses.name as status_id, statuses.description as status_description, habitats.name as habitat, habitats.description as habitat_description").
